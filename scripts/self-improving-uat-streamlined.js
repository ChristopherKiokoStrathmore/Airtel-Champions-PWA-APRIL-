...
async function improveScript(failures, currentScript) {
  // Create a concise prompt with only the failing parts
...
Return ONLY the improved code for the failing functions.
...
- Phone: input[data-testid="phone-input"], input[placeholder*="phone" i], input[type="tel"], input[name*="phone" i]
- PIN: input[data-testid="pin-input"], input[placeholder*="PIN" i], input[type="password"], input[name*="pin" i]
- Submit: button[data-testid="login-submit"], button:has-text("SIGN IN"), button[type="submit"]
- Dashboard: [data-testid="sales-dashboard"], [data-testid="hbb-dashboard"], .dashboard, text=Welcome, text=Dashboard

Return only the improved selector arrays and any additional selectors needed.`;
...
async function applyImprovements(improvedCode) {
  try {
    ...
    // Extract improved selectors from the response
    const selectorsMatch = improvedCode.match(/phoneSelectors\s*=\s*\[([^\]]+)\]/);
    const improvedPhoneSelectors = selectorsMatch ? selectorsMatch[1] : '';
    
    const pinSelectorsMatch = improvedCode.match(/pinSelectors\s*=\s*\[([^\]]+)\]/);
    const improvedPinSelectors = pinSelectorsMatch ? pinSelectorsMatch[1] : '';
    
    const submitSelectorsMatch = improvedCode.match(/submitSelectors\s*=\s*\[([^\]]+)\]/);
    const improvedSubmitSelectors = submitSelectorsMatch ? submitSelectorsMatch[1] : '';
    
    const dashboardSelectorsMatch = improvedCode.match(/dashboardSelectors\s*=\s*\[([^\]]+)\]/);
    const improvedDashboardSelectors = dashboardSelectorsMatch ? dashboardSelectorsMatch[1] : '';
    
    // Update the script with improved selectors
    let updatedScript = currentScript;
    
    if (improvedPhoneSelectors) {
      updatedScript = updatedScript.replace(
        /const phoneSelectors\s*=\s*\[[^\]]+\]/,
        `const phoneSelectors = [${improvedPhoneSelectors}]`
      );
    }
    
    if (improvedPinSelectors) {
      updatedScript = updatedScript.replace(
        /const pinSelectors\s*=\s*\[[^\]]+\]/,
        `const pinSelectors = [${improvedPinSelectors}]`
      );
    }
    
    if (improvedSubmitSelectors) {
      updatedScript = updatedScript.replace(
        /const submitSelectors\s*=\s*\[[^\]]+\]/,
        `const submitSelectors = [${improvedSubmitSelectors}]`
      );
    }
    
    if (improvedDashboardSelectors) {
      updatedScript = updatedScript.replace(
        /const dashboardSelectors\s*=\s*\[[^\]]+\]/,
        `const dashboardSelectors = [${improvedDashboardSelectors}]`
      );
    }
...
async function main() {
  console.log('🚀 Starting STREAMLINED SELF-IMPROVING UAT System');
  console.log('🤖 Will automatically fix its own code when tests fail');
  console.log('🔄 Uses minimal tokens to avoid rate limits');
  ...
  while (iteration < MAX_ITERATIONS && !allPassed) {
    ...
    let improvedCode;
    try {
      improvedCode = await improveScript(failures, '');
    } catch (err) {
